...
| TD-27 | **`/api/health` cannot report a broken database** | ~~Medium~~ **Resolved** | `StatsController.Health()` runs the reachability probe first and wraps probe plus `GetLatestAsync()` in `try`/`catch`. Post-startup probe/query failures return `200` with `database: "disconnected"`. Startup still requires a migratable SQLite file (`Program` calls `Migrate()` before listening). Covered by unit and integration tests for the post-startup path. Closes #27. |
...
2. **TD-09 / TD-10** — route frontend writes through `fetchJson` and extend the dev proxy to `/coffee`. Both are small and each removes a case where the system misreports its own state.
