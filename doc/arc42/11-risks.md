...
| ID | Item | Severity | Detail |
|----|------|----------|--------|
| ~~TD-07~~ | **~~Counter reset is not handled~~** | ~~Medium~~ **Resolved** | A decrease in any cup counter is persisted and treated as a new epoch. Daily and range totals are computed by summing only positive per-pair increments, so the first snapshot after a reset becomes the new baseline. See [ADR-014](09-design.md#adr-014-counter-reset-detection). |
...
| **Counter reset corrupts statistics** | Low | Medium | Silent: numbers stay plausible while being wrong. | Handled by ADR-014: resets are persisted and statistics re-anchor at the first post-reset snapshot. |
...
4. **TD-02** — ApiKey configuration should fail closed in Production instead of silently disabling auth.
